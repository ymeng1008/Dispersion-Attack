...
def get_explanations_nn(model, x_exp, x_back, dataset, knowledge, n_samples=100, load=False, save=False):
    ...
    # Save file name includes the number of background samples
    fname = f'shap_{dataset}_nn_{knowledge}_{x_back.shape[0]}'
    fpath = os.path.join(constants.SAVE_FILES_DIR, fname)
    ...
    contribs = model.explain(
        ...
        X_exp=x_exp,
        n_samples=n_samples
    )[1]  # 假设返回的是一个包含 SHAP 值的列表，其中[1]是对应的贡献值

    # 检查 SHAP 贡献值的形状
    print(f"SHAP contributions shape: {contribs.shape}")
    print(f"Explanation data shape (x_exp): {x_exp.shape}")

    # 检查 SHAP 值的样本数量是否与输入数据的样本数量匹配
    if contribs.shape[0] != x_exp.shape[0]:
        raise ValueError(f"Mismatch in the number of samples between SHAP values ({contribs.shape[0]}) and input data ({x_exp.shape[0]})")

    # 将 SHAP 贡献值存储为 DataFrame，确保列数为特征数量
    shap_values_df = pd.DataFrame(contribs, columns=[f"feature_{i}" for i in range(contribs.shape[1])])
...
def get_explanations_rf(model, x_exp, dataset, knowledge='train', load=False, save=False):
    ...
    fname = f'shap_{dataset}_rf_{knowledge}'
    fpath = os.path.join(constants.SAVE_FILES_DIR, fname)
    ...
    # 打印 SHAP 值的形状，检查是否与输入数据一致
    print(f"SHAP values: {shap_values}")
    print(f"Shape of SHAP values: {[s.shape for s in shap_values]}")

    # 如果是二分类问题，选择第二类的 SHAP 值（假设是二分类）
    if isinstance(shap_values, list):
        shap_values_for_class = shap_values[1]  # 选择第二类的 SHAP 值
    else:
        shap_values_for_class = shap_values  # 对于回归任务，直接使用

    # 将 SHAP 值转换为 DataFrame
    shap_values_df = pd.DataFrame(shap_values_for_class)

    # 调试：输出 SHAP 值的形状，确保与 x_exp 的形状一致
    print(f"SHAP values shape: {shap_values_df.shape}")
    print(f"Input data shape (x_exp): {x_exp.shape}")

    # 检查 SHAP 值的样本数量是否和输入数据一致
    if shap_values_df.shape[0] != x_exp.shape[0]:
        raise ValueError(f"Mismatch in the number of samples between SHAP values ({shap_values_df.shape[0]}) and input data ({x_exp.shape[0]})")
